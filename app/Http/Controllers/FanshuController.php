public function ticketSoldByDateRange(Request $request)
{
			...
			foreach ($order_item as $oi)
            {
                ...
                if($oi->quantity != $count_ticket)//判断是否存在一张ticket文件含有多张票，这个问题坑的不行。。。
                {
                    if($count_ticket == 1)
                    {
			...
			*/
			foreach($ticket as $t){
				...
				if(in_array($t->order->pay_mode_id, $sgd_arr))
                {
                    ...
                    if(isset($many_ticket[$t->id]))
                    {
                        $sgd_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                    }
                    else
                    {
                ...
                elseif(in_array($t->order->pay_mode_id, $rmb_arr))
                {
                    ...
                    if(isset($many_ticket[$t->id]))
                    {
                        $rmb_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                    }
                    else
                    {
                ...
                else
                {
                    if($t->order->pay_mode_id == 12)//stripe
                    {
                        ...
                        if(isset($many_ticket[$t->id]))
                        {
                            $stripe_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                        }
                        else
                        {
                    ...
                    elseif($t->order->pay_mode_id == 11)//PayPal
                    {
                        ...
                        if(isset($many_ticket[$t->id]))
                        {
                            $paypal_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                        }
                        else
                        {
                    ...
                    elseif($t->order->pay_mode_id == 16)//qoo10
                    {
                        ...
                        if(isset($many_ticket[$t->id]))
                        {
                            $qoo10_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                        }
                        else
                        {
                    ...
                    elseif($t->order->pay_mode_id == 18)//shopee
                    {
                        ...
                        if(isset($many_ticket[$t->id]))
                        {
                            $shopee_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                        }
                        else
                        {
                    ...
                    elseif($t->order->pay_mode_id == 19)//sale
                    {
                        ...
                        if(isset($many_ticket[$t->id]))
                        {
                            $sale_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                        }
                        else
                        {
                    ...
                    else//others
                    {
                        ...
                        if(isset($many_ticket[$t->id]))
                        {
                            $others_sale[$t->place_id] += $order_item_detail[$oi_id]['price'] * $order_item_detail[$oi_id]['quantity'];
                        }
                        else
                        {
